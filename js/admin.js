...
// ── ЗАДАЧА 20: Subscription verwalten (Master-Funktion) ─────────────────────

/**
 * Setzt die Subscription einer Praxis.
 * Wird aus der Master-Praxen-Liste aufgerufen.
 *
 * @param {string} praxisId
 * @param {boolean} active        — true = aktivieren, false = deaktivieren
 * @param {string|null} expires   — ISO-Datum 'YYYY-MM-DD' oder null
 * @param {string} plan           — 'Demo'|'Basis'|'Pro'|'Clinic'
 */
function masterSetSubscription(praxisId, active, expires, plan) {
    const p = praxen.find(x => x.id === praxisId);
    if (!p) return;
    p.subscriptionActive  = active;
    p.subscriptionExpires = expires || null;
    p.subscriptionPlan    = plan    || 'Demo';
    saveAll();
    // cu aktualisieren falls aktuelle Praxis betroffen
    if (cu && cu.id === praxisId) {
        cu = { ...cu, subscriptionActive: active, subscriptionExpires: expires, subscriptionPlan: plan };
        DB.set('et2_admin_sess', cu);
    }
    writeAudit(
        `💳 Subscription für ${p.name}: ${active ? plan + ' aktiv' : 'deaktiviert'}` +
        (expires ? ` (bis ${expires})` : ''),
        'master'
    );
    showToast(
        active
            ? `✅ ${p.name}: ${plan}-Abo aktiv${expires ? ' bis ' + expires : ''}!`
            : `🔒 ${p.name}: Abo deaktiviert — Namen werden geblurrt!`,
        active ? 'success' : 'warning'
    );
    renderMasterDashboard();
    if (cu && cu.id === praxisId) renderDashboard();
}

/**
 * Öffnet einen kompakten Subscription-Dialog für eine Praxis.
 */
function masterOpenSubDialog(praxisId) {
    const p = praxen.find(x => x.id === praxisId);
    if (!p) return;

    const sub = isSubscriptionActive(p);
    const todayIso = new Date().toISOString().split('T')[0];

    // Einfaches Prompt-Overlay im Obsidian-Stil
    const existingDlg = document.getElementById('sub-dialog-overlay');
    if (existingDlg) existingDlg.remove();

    const plans = ['Demo', 'Basis (29€)', 'Pro (59€)', 'Clinic (119€)'];
    const planOpts = plans.map(pl => `<option value="${pl}" ${sub.plan === pl ? 'selected' : ''}>${pl}</option>`).join('');

    const dlgHtml = `
    <div id="sub-dialog-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.8);backdrop-filter:blur(10px);z-index:10030;display:flex;align-items:center;justify-content:center;">
        <div style="background:#0f0d14;border:1px solid rgba(255,215,0,.3);border-radius:18px;padding:32px 28px;width:92%;max-width:420px;box-shadow:0 0 60px rgba(255,215,0,.08);">
            <div style="font-family:'Fraunces',serif;font-size:1.2rem;font-weight:900;color:#ffd700;margin-bottom:4px;">💳 Abonnement verwalten</div>
            <div style="font-size:.8rem;color:rgba(255,215,0,.5);margin-bottom:22px;">${p.name}</div>

            <label style="color:rgba(255,215,0,.7);font-size:.78rem;font-weight:600;display:block;margin-bottom:6px;">Plan</label>
            <select id="sub-dlg-plan" style="width:100%;padding:9px 12px;background:#1a1625;border:1px solid rgba(255,215,0,.25);border-radius:10px;color:#ffd700;font-size:.88rem;margin-bottom:14px;">
                ${planOpts}
            </select>

            <label style="color:rgba(255,215,0,.7);font-size:.78rem;font-weight:600;display:block;margin-bottom:6px;">Läuft ab am (leer = unbegrenzt)</label>
            <input type="date" id="sub-dlg-expires" value="${p.subscriptionExpires || ''}"
                style="width:100%;padding:9px 12px;background:#1a1625;border:1px solid rgba(255,215,0,.25);border-radius:10px;color:#ffd700;font-size:.88rem;margin-bottom:20px;box-sizing:border-box;">

            <div style="display:flex;gap:10px;">
                <button onclick="masterSetSubscription('${praxisId}',true,document.getElementById('sub-dlg-expires').value||null,document.getElementById('sub-dlg-plan').value);document.getElementById('sub-dialog-overlay').remove();"
                    style="flex:1;background:linear-gradient(135deg,#27ae60,#1a7a3f);border:none;border-radius:10px;color:#fff;font-weight:700;font-size:.88rem;padding:12px;cursor:pointer;">
                    ✅ Aktivieren
                </button>
                <button onclick="masterSetSubscription('${praxisId}',false,null,'Demo');document.getElementById('sub-dialog-overlay').remove();"
                    style="flex:1;background:rgba(231,76,60,.15);border:1px solid rgba(231,76,60,.4);border-radius:10px;color:#ff6b6b;font-weight:700;font-size:.88rem;padding:12px;cursor:pointer;">
                    🔒 Deaktivieren
                </button>
                <button onclick="document.getElementById('sub-dialog-overlay').remove();"
                    style="background:rgba(255,215,0,.08);border:1px solid rgba(255,215,0,.2);border-radius:10px;color:rgba(255,215,0,.5);font-size:.88rem;padding:12px 16px;cursor:pointer;">
                    ✕
                </button>
            </div>
        </div>
...
// ── ЗАДАЧА 20: Subscription-Banner für eigene Praxis (Arzt sieht es) ─────────
function renderSubscriptionBanner() {
    const wrap = document.getElementById('subscription-banner-wrap');
    if (!wrap || !cu) return;
    const sub = isSubscriptionActive(cu);
    if (sub.active && !sub.warn) { wrap.innerHTML = ''; return; }

    let html = '';
    if (sub.expired) {
        html = `<div class="sub-expired-banner">
            <div class="sub-banner-icon">🔒</div>
            <div class="sub-banner-text">
                <strong>Abonnement abgelaufen!</strong><br>
                <span>Patientennamen werden aus Datenschutzgründen verborgen. Bitte Abo verlängern.</span>
            </div>
            <button class="sub-banner-btn" onclick="showToast('Bitte kontaktieren Sie support@einfach-termin.de', 'info')">Verlängern →</button>
        </div>`;
    } else if (sub.warn) {
        html = `<div class="sub-expired-banner warn">
            <div class="sub-banner-icon">⚠️</div>
            <div class="sub-banner-text">
                <strong>Abo läuft in ${sub.daysLeft} Tag${sub.daysLeft !== 1 ? 'en' : ''} ab!</strong><br>
                <span>Plan: ${sub.plan} · Ablauf: ${cu.subscriptionExpires}</span>
            </div>
            <button class="sub-banner-btn" onclick="showToast('Bitte kontaktieren Sie support@einfach-termin.de', 'info')">Jetzt verlängern →</button>
        </div>`;
    }
    wrap.innerHTML = html;
}

// ── ЗАДАЧА 20: Blur in Tagesplan drucken ──────────────────────────────────────
function subBlurNamePrint(name) {
    return subBlurName(name, cu, false); // kein HTML, nur Text
}
    ...
    if (listEl) {
        if (!praxen.length) {
            ...
            listEl.innerHTML = praxen.map(function(p) {
                ...
                var subFn    = 'masterOpenSubDialog(\'' + p.id + '\')';
                var hasScan  = !!(p.approbationScan && p.approbationScan.base64);
                // ── ЗАДАЧА 20: Subscription-Status ──
                var subInfo  = isSubscriptionActive(p);
                var subLabel = subInfo.expired
                    ? '<span style="color:#ff6b6b;font-size:.68rem;font-weight:700;">🔒 Abo abgelaufen</span>'
                    : subInfo.warn
                        ? '<span style="color:#f39c12;font-size:.68rem;font-weight:700;">⚠️ ' + subInfo.daysLeft + ' Tage</span>'
                        : '<span style="color:#2ecc71;font-size:.68rem;">💳 ' + subInfo.plan + '</span>';
                var actionBtns =
                    (isPending
                    ...
                    '<button class="master-action-btn" onclick="' + subFn + '" title="Abonnement verwalten" style="color:rgba(255,215,0,.7);">💳</button>' +
...
async function handleRegister() {
    ...
    if (!isLANRValid(bsnr)) {
        errEl.innerHTML = '❌ <strong>BSNR</strong> muss genau 9 Ziffern haben!';
        errEl.style.display = 'block';
        document.getElementById('r-bsnr')?.focus();
        // ── ЗАДАЧА 22: Deep Audit — LANR-Fehler ──────────────────────────────
        writeAudit(
            '⚠️ BSNR-FEHLER: Registrierungsversuch mit ungültiger BSNR "' + bsnr + '" für ' + (document.getElementById('r-name')?.value || '?'),
            null,
            { category: 'lanr', severity: 'warn', meta: { bsnr, email, praxisName: document.getElementById('r-name')?.value } }
        );
        return;
    }
    if (!isLANRValid(lanr)) {
        errEl.innerHTML = '❌ <strong>LANR</strong> muss genau 9 Ziffern haben!';
        errEl.style.display = 'block';
        document.getElementById('r-lanr')?.focus();
        // ── ЗАДАЧА 22: Deep Audit — LANR-Fehler ──────────────────────────────
        writeAudit(
            '⚠️ LANR-FEHLER: Registrierungsversuch mit ungültiger LANR "' + lanr + '" für ' + (document.getElementById('r-name')?.value || '?'),
            null,
            { category: 'lanr', severity: 'warn', meta: { lanr, email, praxisName: document.getElementById('r-name')?.value } }
        );
        return;
    }
    if (praxen.find(p => p.bsnr === bsnr)) {
        errEl.innerHTML = '❌ Diese <strong>BSNR</strong> ist bereits registriert!';
        errEl.style.display = 'block';
        // ── ЗАДАЧА 22: Deep Audit — дублирующая BSNR ─────────────────────────
        writeAudit(
            '🚨 BSNR-DUPLIKAT: Versuch, bereits registrierte BSNR "' + bsnr + '" erneut zu verwenden (' + email + ')',
            null,
            { category: 'lanr', severity: 'error', meta: { bsnr, email } }
        );
        return;
    }
...
function renderDashboard() {
    ...
    // ── ЗАДАЧА 30: Direktlink mit ?praxis=slug ────────────────────
    const baseUrl  = window.location.origin + window.location.pathname.replace('admin.html','index.html');
    const link     = `${baseUrl}?praxis=${cu.slug}`;
    const linkDisplay = `${window.location.hostname}/index.html?praxis=${cu.slug}`;
    const linkBar  = document.getElementById('praxis-link-bar');
    if (linkBar) linkBar.innerHTML =
        `<span style="font-size:.82rem;color:var(--text-muted);">Ihr Buchungslink:</span>` +
        `<span class="praxis-link-pill" onclick="copyDirectLink('${link}')" title="Klicken zum Kopieren">🔗 ${linkDisplay}</span>` +
        `<button class="btn-sm" style="border-color:var(--primary);color:var(--primary);" onclick="copyDirectLink('${link}')">📋 Kopieren</button>` +
        `<button class="btn-sm" style="border-color:var(--success);color:var(--success);" onclick="window.open('${link}','_blank')" title="Link in neuem Tab öffnen">↗️ Testen</button>`;
    ...
    initManualBooking();
    renderNoShowPanel();
    renderSubscriptionBanner(); // ЗАДАЧА 20
    loadEJSConfigFields();
    renderAuslastung();
}

// ── ЗАДАЧА 4 + 21: Виджеты-карточки раздела Обзор ───────────────────────────
function renderOverviewWidgets(myAppts, myDocs) {
    ...
    // ── ЗАДАЧА 21: Полные данные подписки через isSubscriptionActive ──────────
    const subInfo    = isSubscriptionActive(cu);
    const subPlan    = subInfo.plan;
    const subExpired = subInfo.expired;
    const subWarn    = subInfo.warn;
    const subDaysLeft= subInfo.daysLeft;
    ...
    // ── ЗАДАЧА 21: Abo-Widget — детальная карточка ─────────────────────────────
    const subClass = subExpired ? 'ow-card ow-subscription expired' : subWarn ? 'ow-card ow-subscription warn' : 'ow-card ow-subscription';

    // Прогресс срока (визуальный индикатор оставшегося времени)
    let subProgressHtml = '';
    if (subExpires && !subExpired) {
        const createdAt  = cu.subscribedAt || cu.verifiedAt || null;
        let totalDays    = 30; // fallback
        if (createdAt) {
            totalDays = Math.max(1, Math.ceil((new Date(subExpires) - new Date(createdAt)) / 86400000));
        }
        const usedDays   = Math.max(0, totalDays - (subDaysLeft || 0));
        const usedPct    = Math.min(100, Math.round(usedDays / totalDays * 100));
        const barColor   = subWarn ? 'var(--danger)' : usedPct > 70 ? 'var(--warning)' : 'var(--success)';
        subProgressHtml  = '<div class="ow-progress" style="margin-top:8px;" title="' + usedDays + ' von ' + totalDays + ' Tagen verbraucht">' +
                           '<div class="ow-progress-bar" style="width:' + usedPct + '%;background:' + barColor + ';"></div>' +
                           '</div>';
    }

    // Фичи плана
    const planFeatures = {
        'Demo':         '1 Arzt · 30 Termine/Monat',
        'Basis (29€)':  'Bis 5 Ärzte · E-Mail · Kanban',
        'Pro (59€)':    'SMS · Analytik · PVS-Export',
        'Clinic (119€)':'Unbegrenzt · Firebase · API'
    };
    const planFeat = planFeatures[subPlan] || '—';

    // Метка статуса
    let subStatusBadge;
    if (subExpired)       subStatusBadge = '<div class="ow-badge danger">❌ Abgelaufen</div>';
    else if (subWarn)     subStatusBadge = '<div class="ow-badge warn">⚠️ Noch ' + subDaysLeft + ' Tage</div>';
    else if (!subExpires) subStatusBadge = '<div class="ow-badge ok">∞ Unbegrenzt</div>';
    else                  subStatusBadge = '<div class="ow-badge ok">✅ ' + subDaysLeft + ' Tage übrig</div>';

    // Текст под планом
    const subSubtext = subExpires
        ? 'Läuft ab: <strong>' + subExpires + '</strong>'
        : '<em>Unbegrenzt (Demo)</em>';

    // Кнопка «Verlängern»
    const renewBtn = '<button class="ow-renew-btn" onclick="openSubscriptionModal()" title="Abonnement verlängern oder upgraden">' +
        (subExpired ? '🔓 Jetzt freischalten' : '🔄 Verlängern / Upgrade') +
        '</button>';
    ...
    wrap.innerHTML =
        ...
        // ── ЗАДАЧА 21: Полноценная карточка подписки ───────────────────────
        '<div class="' + subClass + '">' +
            '<div class="ow-icon">💳</div>' +
            '<div class="ow-value ow-sub-plan">' + subPlan + '</div>' +
            '<div class="ow-label">Abonnement</div>' +
            '<div class="ow-sub" style="line-height:1.5;">' + subSubtext + '<br>' +
                '<span style="font-size:.7rem;color:var(--text-muted);">' + planFeat + '</span>' +
            '</div>' +
            subStatusBadge +
            subProgressHtml +
            renewBtn +
...
// ── ЗАДАЧА 21: Abonnement-Modal (Verlängern / Upgrade) ───────────────────────
function openSubscriptionModal() {
    const existingModal = document.getElementById('sub-modal-overlay');
    if (existingModal) existingModal.remove();

    const sub       = isSubscriptionActive(cu);
    const curPlan   = sub.plan;
    const curExpiry = cu.subscriptionExpires || null;
    ...
    const plans = [
        {
            ...
            price: 'Kostenlos',
            color: '#888',
            features: ['1 Arzt', '30 Termine/Monat', 'Kein E-Mail', 'Basis-Ansicht'],
            highlight: false
        },
        {
            id: 'Basis (29€)',
            ...
            price: '29 €/Monat',
            color: '#4b7bec',
            features: ['Bis 5 Ärzte', 'E-Mail-Benachrichtigungen', 'Kanban-Board', 'Terminexport CSV'],
            highlight: false
        },
        {
            id: 'Pro (59€)',
            ...
            price: '59 €/Monat',
            color: '#7c5cbf',
            features: ['Unbegrenzte Ärzte', 'SMS-Erinnerungen', 'Analytik & Heatmap', 'PVS-Export', 'Prioritätssupport'],
            highlight: true   // empfohlen
        },
        {
            id: 'Clinic (119€)',
            label: 'Clinic',
            price: '119 €/Monat',
            color: '#f39c12',
            features: ['Alles in Pro', 'Firebase Echtzeit-Sync', 'API-Zugang', 'Mehrere Standorte', 'Dedizierter Support'],
            highlight: false
        }
    ];

    const planCards = plans.map(p => {
        const isCurrent = p.id === curPlan;
        const feats = p.features.map(f => '<li>✓ ' + f + '</li>').join('');
        return `<div class="sub-plan-card${p.highlight ? ' recommended' : ''}${isCurrent ? ' current' : ''}"
            onclick="selectSubPlan('${p.id}', this)"
            data-plan="${p.id}">
            ${p.highlight ? '<div class="sub-plan-ribbon">⭐ Empfohlen</div>' : ''}
            ${isCurrent ? '<div class="sub-plan-ribbon current-ribbon">✓ Aktuell</div>' : ''}
            <div class="sub-plan-icon" style="background:${p.color}20;color:${p.color};">
                ${p.label.charAt(0)}
            </div>
            <div class="sub-plan-name" style="color:${p.color};">${p.label}</div>
            <div class="sub-plan-price">${p.price}</div>
            <ul class="sub-plan-features">${feats}</ul>
        </div>`;
    }).join('');

    // Дата продления — по умолчанию +30 дней от сегодня или от истечения
    const baseDate = (curExpiry && !sub.expired) ? new Date(curExpiry) : new Date();
    baseDate.setDate(baseDate.getDate() + 30);
    const defaultExpiry = baseDate.toISOString().split('T')[0];

    const modalHtml = `
    <div id="sub-modal-overlay" style="position:fixed;inset:0;background:rgba(44,27,105,.55);backdrop-filter:blur(8px);z-index:10025;display:flex;align-items:center;justify-content:center;padding:16px;">
        <div class="sub-modal-box">
            <!-- Header -->
            <div class="sub-modal-header">
                <div>
                    <div class="sub-modal-title">💳 Abonnement</div>
                    <div class="sub-modal-subtitle">Wählen Sie Ihren Plan · Vergleich mit Doctolib: ab 300 €/Monat</div>
                </div>
                <button onclick="document.getElementById('sub-modal-overlay').remove()"
                    class="sub-modal-close">✕</button>
            </div>

            <!-- Status aktuell -->
            <div class="sub-modal-current-status">
                <span class="sub-modal-status-icon">${sub.expired ? '🔒' : '✅'}</span>
                <span>
                    <strong>Aktuell: ${curPlan}</strong>
                    ${curExpiry ? ' · Läuft ab: ' + curExpiry : ' · Unbegrenzt'}
                    ${sub.expired ? ' · <span style="color:var(--danger);">Abgelaufen!</span>' : ''}
                    ${sub.warn ? ' · <span style="color:var(--warning);">Nur noch ' + sub.daysLeft + ' Tage!</span>' : ''}
                </span>
            </div>

            <!-- Plan-Karten -->
            <div class="sub-plans-grid" id="sub-plans-grid">
                ${planCards}
            </div>

            <!-- Ablaufdatum wählen -->
            <div class="sub-modal-expiry-row">
                <label class="sub-modal-expiry-label">📅 Neues Ablaufdatum (leer = unbegrenzt)</label>
                <input type="date" id="sub-modal-expiry" value="${defaultExpiry}" class="sub-modal-expiry-input">
                <div class="sub-modal-expiry-shortcuts">
                    <button onclick="setSubExpiry(30)">+30 Tage</button>
                    <button onclick="setSubExpiry(90)">+90 Tage</button>
                    <button onclick="setSubExpiry(180)">+180 Tage</button>
                    <button onclick="setSubExpiry(365)">+1 Jahr</button>
                    <button onclick="setSubExpiry(0)" style="color:var(--text-muted);">Unbegrenzt</button>
                </div>
            </div>
            ...
            <div class="sub-modal-footer">
                <div style="font-size:.75rem;color:var(--text-muted);line-height:1.5;">
                    💡 In dieser Demo-Version wird die Subscription lokal gespeichert.<br>
                    Für Produktion: Stripe-Integration via <strong>support@einfach-termin.de</strong>
                </div>
                <div style="display:flex;gap:10px;">
                    <button onclick="document.getElementById('sub-modal-overlay').remove()"
                        class="btn btn-outline" style="width:auto;padding:10px 20px;margin:0;">Abbrechen</button>
                    <button onclick="applySubPlanFromModal()"
                        class="btn btn-main" style="width:auto;padding:10px 24px;margin:0;">
                        💳 Plan aktivieren
                    </button>
                </div>
            </div>
    ...
    // Выделяем текущий план
    const curCard = document.querySelector(`[data-plan="${curPlan}"]`);
    if (curCard) curCard.classList.add('selected');
}

let _selectedSubPlan = null;

function selectSubPlan(planId, el) {
    _selectedSubPlan = planId;
    document.querySelectorAll('.sub-plan-card').forEach(c => c.classList.remove('selected'));
    if (el) el.classList.add('selected');
}

function setSubExpiry(days) {
    const inp = document.getElementById('sub-modal-expiry');
    if (!inp) return;
    if (days === 0) { inp.value = ''; return; }
    const base = (cu.subscriptionExpires && !isSubscriptionActive(cu).expired)
        ? new Date(cu.subscriptionExpires) : new Date();
    base.setDate(base.getDate() + days);
    inp.value = base.toISOString().split('T')[0];
}

function applySubPlanFromModal() {
    const plan   = _selectedSubPlan || cu.subscriptionPlan || 'Demo';
    const expiry = document.getElementById('sub-modal-expiry')?.value || null;

    // Сохраняем в praxis + cu
    const p = praxen.find(x => x.id === cu.id);
    if (p) {
        p.subscriptionActive  = true;
        p.subscriptionExpires = expiry || null;
        p.subscriptionPlan    = plan;
        p.subscribedAt        = p.subscribedAt || new Date().toISOString().split('T')[0];
    }
    cu.subscriptionActive  = true;
    cu.subscriptionExpires = expiry || null;
    cu.subscriptionPlan    = plan;
    cu.subscribedAt        = cu.subscribedAt || new Date().toISOString().split('T')[0];
    DB.set('et2_admin_sess', cu);

    saveAll();
    writeAudit(`💳 Abo geändert: ${plan}${expiry ? ' bis ' + expiry : ' (unbegrenzt)'}`, cu.id);
    showToast(`✅ Plan "${plan}" aktiviert${expiry ? ' bis ' + expiry : ' (unbegrenzt)'}!`, 'success');
    SoundUX.confirm();
...
function saveDocFormModal() {
    ...
    if (!id && !isLANRValid(lanr)) {
        ...
        // ── ЗАДАЧА 22: Deep Audit — LANR-Fehler bei Arzt-Hinzufügen ──────────
        writeAudit(
            '⚠️ LANR-FEHLER: Ungültige LANR "' + lanr + '" beim Hinzufügen von Arzt "' + name + '"',
            cu?.id,
            { category: 'lanr', severity: 'warn', meta: { lanr, doctorName: name, spec } }
        );
        return;
    }
    ...
    if (lanr && isLANRValid(lanr)) {
        ...
        if (duplicate) {
            ...
            errEl.style.display = 'block';
            // ── ЗАДАЧА 22: Deep Audit — дублирующая LANR ─────────────────────
            writeAudit(
                '🚨 LANR-DUPLIKAT: LANR "' + lanr + '" für "' + name + '" bereits bei ' + duplicate.name,
                cu?.id,
                { category: 'lanr', severity: 'error', meta: { lanr, doctorName: name, existingDoctor: duplicate.name } }
            );
            return;
        }
    }
...
function kanbanCard(a, bl) {
    ...
    // ── ЗАДАЧА 20: Blur-Schutz bei abgelaufener Subscription ──
    const myPraxis   = praxen.find(p => p.id === cu?.id) || cu;
    const nameHtml   = subBlurName(a.patientName, myPraxis, true);
...
function printTodaySchedule() {
    ...
    const sections = Object.values(byDoctor).map(({ doc, appts: da }) => {
        const rows = da.map(a => `<tr class="${a.urgent ? 'sp-urgent' : a.isManual ? 'sp-manual' : ''}"><td><strong>${a.time}</strong></td><td>${subBlurNamePrint(a.patientName)}${a.urgent ? ' ⚠️' : ''}${a.isManual ? ' 📝' : ''}</td><td>${typeLabels[a.apptType] || a.apptType}</td><td>${a.reason || '—'}</td><td>${a.note ? a.note.slice(0,30) : ''}${a.privateNote ? '<br><span style="font-size:.7rem;color:#e65100;">[🔒 '+a.privateNote.slice(0,30)+']</span>' : ''}${!a.note&&!a.privateNote?'—':''}</td><td>${statusLabels[a.status] || a.status}</td><td style="width:60px;border:1px solid #ccc;"></td></tr>`).join('');
...
// ── ЗАДАЧА 30: Direktlink kopieren ───────────────────────────────
/**
 * Kopiert den vollständigen Direktlink in die Zwischenablage.
 * Unterstützt Web Share API auf Mobilgeräten.
 */
function copyDirectLink(url) {
    // Web Share API (nur auf Mobilgeräten verfügbar)
    if (navigator.share && /Mobi|Android/i.test(navigator.userAgent)) {
        navigator.share({
            title: cu?.name || 'Einfach-Termin',
            text:  'Direkt online einen Termin buchen bei ' + (cu?.name || 'unserer Praxis'),
            url:   url
...
function _copyToClipboard(text) {
    if (navigator.clipboard) {
        navigator.clipboard.writeText(text)
            .then(() => showToast('🔗 Buchungslink kopiert!', 'success'))
...
function _fallbackCopy(text) {
    ...
    try { document.execCommand('copy'); showToast('🔗 Link kopiert!', 'success'); }
...
// ── Audit Log ────────────────────────────────────────────────────
// ── ЗАДАЧА 22: Deep Audit — расширенный renderAuditLog ───────────────────────
...
function renderAuditLog() {
    ...
    // ── Фильтр по категории ──────────────────────────────────────────────────
    const filterEl  = document.getElementById('audit-filter-cat');
    const filterCat = filterEl ? filterEl.value : '';
    const filtered  = filterCat ? myLog.filter(e => e.category === filterCat) : myLog;

    if (!filtered.length) {
        wrap.innerHTML = '<div class="empty" style="font-size:.8rem;">Noch keine Aktionen' + (filterCat ? ' in dieser Kategorie' : '') + '.</div>';
        return;
    }

    wrap.innerHTML = filtered.map(e => {
        const catIcon  = AUDIT_CATEGORY_ICONS[e.category] || '📋';
        const sevColor = AUDIT_SEVERITY_COLORS[e.severity] || AUDIT_SEVERITY_COLORS.info;
        const metaHtml = e.meta ? `<div class="audit-meta">${
            Object.entries(e.meta)
                .filter(([k, v]) => v !== null && v !== undefined && v !== '')
                .map(([k, v]) => `<span class="audit-meta-tag"><span class="audit-meta-key">${k}</span>${v}</span>`)
                .join('')
        }</div>` : '';
        return `<div class="audit-item audit-item--${e.severity || 'info'}">
            <div class="audit-item-left">
                <span class="audit-cat-icon" title="${e.category || 'system'}">${catIcon}</span>
                <div class="audit-item-body">
                    <span class="audit-message" style="border-left:2px solid ${sevColor};padding-left:6px;">${e.message}</span>
                    ${metaHtml}
                </div>
            </div>
